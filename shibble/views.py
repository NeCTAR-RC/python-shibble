...
from shibble.models import (User, create_shibboleth_user,
...
def root(db):
    ...
    if not shib_user:
        shib_user = create_shibboleth_user(db, shib_attrs)
    ...
    if request.forms.get("agree") and shib_user.state == "new":
        ...
        update_shibboleth_user(db, shib_user, shib_attrs)
        db.commit()
        ...
        try:
            ...
            data = {
                ...
                "message": "There was an error creating your local account",
                "errors": [str(e)],
            }
            return template("error", **data)
